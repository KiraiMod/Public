...
namespace KiraiMod.Managers
{
    public static class GUIManager
    {
        static GUIManager()
        {
            ...
            if (Plugin.Configuration.Bind("GUI", "SideUI", true, "Should the GUI manager automatically start up the SideUI adapter?").Value)
        ...
        private static void SetupHandlers()
        {
            ...
            for (int i = 0; i < Core.UI.LegacyGUIManager.UserInterface.transform.childCount; i++)
            {
                ...
                if (handler == null)
                {
                    Plugin.Logger.LogWarning($"Failed to find GUI handler for {child.name}");
                    continue;
                }
                ...
                if (setup == null)
                {
                    Plugin.Logger.LogWarning($"Missing setup method for GUI handler {child.name}");
                    continue;
                }
                ...
                {
                    Plugin.Logger.LogError($"Setup method for GUI handler {child.name} failed: {ex}");
            ...
            Plugin.Logger.LogInfo($"Setup GUI in {sw.Elapsed.Milliseconds} ms");
        }
